...
const { get: getKeypath } = lodash

import coreFilters from './core_filters.mjs'
import { findAvailablePort } from './utils/find-available-port.mjs'
 ...
 */
export async function addNunjucksFilters(env) {
  // Use dynamic import for CommonJS module
  const { default: customFilters } = await import('../app/filters.js')
 ...
 * @param {Environment} env
 */
export function addCheckedFunction(env) {
 ...
 * @param {unknown} data
 */
export function storeData(input, data) {
...
// Load session data defaults asynchronously
async function loadSessionDataDefaults() {
  try {
    // Use dynamic import for CommonJS module
    const { default: defaults } = await import(sessionDataDefaultsFile)
    sessionDataDefaults = defaults
  } catch (error) {
    console.error(
...
// Initialize session data defaults
loadSessionDataDefaults()
