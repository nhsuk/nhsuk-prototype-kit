...
router.get('/reset', (req, res) => {
  const backHref = req.get('Referrer') || '/';
  res.render('reset', {
    backHref,
  });
});
...
router.all('/reset-session-data', (req, res) => {
  req.session.data = {};
  const returnPath = req.body.previousPath || '/';
  const successPath = '/prototype-admin/reset-session-data-success';
  res.redirect(`${successPath}?returnPath=${returnPath}`);
});
