...
router.post("/v6/patient/change-due-date/change", function(req, res) {
  ...
  if (
    ...
    // Send user to ineligible page
    res.redirect("/v6/patient/change-due-date/enter-reason-check-defer");
  }
});

router.post("/v7/patient/change-due-date/change", function(req, res) {
  // Make a variable and give it the value from 'know-nhs-number'
  var reason = req.session.data["reason"];
  // Check whether the variable matches a condition
  ...
  if (
    ...
    // Send user to next page
    res.redirect("/v7/patient/change-due-date/enter-reason-check-cease");
  } else {
    // Send user to ineligible page
    res.redirect("/v7/patient/change-due-date/enter-reason-check-defer");
  }
});
...
router.post("/v7/change-due-date", function(req, res) {
  if (req.session.data["recall"] == "defer") {
    res.redirect("/v7/patient/patient-summary-deferred");
  }

  if (req.session.data["recall"] == "cease") {
    res.redirect("/v7/patient/patient-summary-ceased");
  }
...
router.post("/search-v7/", function(req, res) {
  var nhsNumber = req.session.data["searchnhs"];
  ...
  if (nhsNumber == "6170211547") {
    ...
    req.session.data["status"] = "ROUTINE";
    req.session.data["address"] = "19 Polly Fall Close, Bradford, BD10 3RT";
    res.redirect("/v7/patient/patient-summary");
  }
